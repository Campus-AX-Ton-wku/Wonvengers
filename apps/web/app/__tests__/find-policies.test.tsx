...
describe("/find/policies", () => {
  it("답변을 모두 모름으로 둬도 목록이 뜬다", async () => {
    ...
    // 남는다 — 18건 중 마감이 아닌 10건. 제목은 지금 신청할 수 있는 것만 센다.
  ...
  it("제목의 건수와 펼쳐진 카드 수가 같다", async () => {
    ...
    // 마감), 해당 없음 15건(전세 전용인 보증료 지원·서울·울산·인천(시·구)·제주·
    // 부산·세종·광주 서구 전용 정책 포함)
    expect(screen.getByRole("heading", { level: 1 }).textContent).toBe(
      "받을 수 있는 주거 혜택 2개"
    );
    ...
    expect(screen.getByText("신청할 수 없는 지원금 16개")).toBeTruthy();
  });
