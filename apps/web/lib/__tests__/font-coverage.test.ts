...
describe("폰트 서브셋", () => {
  ...
  /* 서브셋이 비어 있거나 라틴만 있으면 위 검사가 통과할 수 없다. 그래도 파일이
     잘못 생성됐을 때 원인이 '글자 N개 빠짐' 으로 보이면 헷갈리므로 따로 본다.
     PretendardStd(한글 0자)를 잘못 넣은 적이 있다. */
  it("서브셋 목록이 한글을 담고 있다 — 라틴 전용 파일을 잘못 넣지 않았는지", () => {
    expect(한글만(readFileSync(CHARSET, "utf-8")).size).toBeGreaterThan(400);
  });

  /*
   * 상용 한글(KS X 1001 2,350자)까지 넓혀 둔다.
   *
   * 앱이 실제로 쓰는 글자만 담았을 때는 **주석을 고쳐도** 테스트가 실패했다.
   * 이 레포는 주석이 한글로 두껍게 달려 있어 자주 걸렸고, 재생성에는 fontTools·
   * brotli 와 원본 폰트 2MB 가 필요해서 그 도구가 없는 사람은 빨간 테스트를
   * 스스로 풀 수 없었다. 290KB 를 더 내고 그 상황을 없앤다 (144KB → 434KB).
   *
   * 넓혀도 이 파일의 위 검사는 남긴다 — 서브셋 파일이 잘못 교체되는 사고(라틴
   * 전용 판을 넣는 것)를 잡는 값은 그대로다.
   */
  it("상용 한글까지 담고 있다 — 주석을 고쳐도 걸리지 않게", () => {
    const 서브셋 = 한글만(readFileSync(CHARSET, "utf-8"));

    expect(서브셋.size).toBeGreaterThanOrEqual(2350);
  });

  it("앱에 안 쓰이는 상용 한글도 들어 있다", () => {
    const 서브셋 = 한글만(readFileSync(CHARSET, "utf-8"));

    // KS X 1001 안에 있고 앱 텍스트에는 없는 글자들. 넓히기 전에는 전부 빠져 있었다.
    for (const 글자 of [..."괜뼘삯넋곗휑앎짢촘뜀얹궂섧젠"]) {
      expect(서브셋.has(글자), `'${글자}' 가 서브셋에 없다`).toBe(true);
    }
  });

  /* 현대 한글 전체(11,172자)로 가면 1,694KB 다. 상용 한글에서 멈춘 선을 고정한다. */
  it("현대 한글 전체까지 담지는 않는다 — 1,694KB 가 된다", () => {
    const 서브셋 = 한글만(readFileSync(CHARSET, "utf-8"));

    expect(서브셋.size).toBeLessThan(11172);
    expect(서브셋.has("뷁")).toBe(false); // KS X 1001 밖
  });
});
